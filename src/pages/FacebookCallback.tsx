...
const FacebookCallback: React.FC = () => {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        const { code, state, error } = facebookAuthService.handleCallback();
        ...
        if (!code || !state) {
          throw new Error('No authorization code or state received from Facebook');
        }

        const userData = await facebookAuthService.authenticateUser(code, state);
        ...
        setError(error instanceof Error ? error.message : 'Facebook authentication failed');
